...
class AnimateFromCoeff():
    ...
    def generate(self, x, video_save_dir, pic_path, crop_info, enhancer=None, background_enhancer=None, preprocess='crop', img_size=256):
        ...
        if enhancer:
            ...
            # We only care about the enhanced result now; remove the
            # intermediate non‑enhanced videos from this run folder.
            try:
                if os.path.isfile(full_video_path) and full_video_path != av_path_enhancer:
                    os.remove(full_video_path)
            except OSError:
                pass
            try:
                if os.path.isfile(av_path) and av_path not in (av_path_enhancer, full_video_path):
                    os.remove(av_path)
            except OSError:
                pass
